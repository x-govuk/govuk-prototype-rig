...
/**
 * Create a heading with GOV.UK typography classes.
 *
 * @see {@link https://design-system.service.gov.uk/styles/typography/#headings}
 *
 * @param {Object} options - Component options
 * @return {string} A heading (and caption) tag with the correct typography classes
 */
export function govukHeading (options) {
  ...
  const captionHtml = options.caption ? `<span class="govuk-caption-${options.size}">${options.caption}</span>` : ''
  const headingOpenHtml = `<h${options.headingLevel} class="govuk-heading-${options.size}">`
  const headingCloseHtml = `</h${options.headingLevel}>`

  let html
  if (options.combine) {
    html = `${headingOpenHtml}\n${captionHtml}\n${options.text}\n${headingCloseHtml}`
  } else {
    html = `${captionHtml}\n${headingOpenHtml}${options.text}${headingCloseHtml}`
  }
...
/**
 * Log an object to the JavaScript console.
 *
